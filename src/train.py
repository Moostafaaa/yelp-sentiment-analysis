...
def main(): 
    ...
    # Path for logging (kept in the main YELP folder)
    log_file = '/content/drive/MyDrive/YELP/experiments_log.csv'

    # --- PATH 1: TF-IDF + Logistic Regression ---
    if args.model_type == "tfidf":
        print(f"--- Starting TF-IDF Baseline on {args.dataset} ---")
        
        train_texts = dataset['train'][text_column][:args.subset_size]
        train_labels = dataset['train']['label'][:args.subset_size]
        test_texts = dataset['test'][text_column][:int(args.subset_size * 0.3)]
        test_labels = dataset['test']['label'][:int(args.subset_size * 0.3)]
        ...
        log_data = {
            "experiment_name": f"LR_{args.dataset}_{args.subset_size}",
    ...
    # --- PATH 2: Transformer Fine-Tuning ---
        ...
        print(f"--- Starting Transformer Fine-tuning on {args.dataset} ---")
        
        train_ds = dataset["train"].shuffle(seed=42).select(range(args.subset_size))
        test_ds = dataset["test"].shuffle(seed=42).select(range(int(args.subset_size * 0.3)))
        ...
        # Save weights and tokenizer to output_dir
        print(f"Saving model weights to {args.output_dir}...")
        ...
        log_data = {
            "experiment_name": f"Transformer_{args.dataset}_{args.subset_size}",
        ...
        pd.DataFrame([log_data]).to_csv(log_file, mode='a', header=not os.path.exists(log_file), index=False)
        print(f"Transformer Complete. Model saved and F1 logged.")
